 ...
 function encode_state(s::GameState)::Matrix{Float32}
     # Pre-allocate a single matrix to minimize allocations.
     # Shape: (4 channels, 12 pits)
     x = Matrix{Float32}(undef, 4, 12)

     # Channel 1: Normalized Seeds
     for i in 1:12
         x[1, i] = Float32(s.board[i]) / 48.0f0
     end

     # Channel 2: Active Player (1.0 for P1, 0.0 for P2)
     player_val = s.to_move == 1 ? 1.0f0 : 0.0f0
     for i in 1:12
         x[2, i] = player_val
     end

     # Channel 3: Captures Player 1
     cap1 = Float32(s.captured[1]) / 48.0f0
     for i in 1:12
         x[3, i] = cap1
     end

     # Channel 4: Captures Player 2

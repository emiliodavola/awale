...
function sample_batch(
    ...
    0.0 <= recent_fraction <= 1.0 ||
        throw(ArgumentError("recent_fraction must be between 0 and 1"))
    recent_window >= 0 || throw(ArgumentError("recent_window must be >= 0"))
    recent_fraction == 0.0 ||
        recent_window > 0 ||
        throw(ArgumentError("recent_window must be > 0 when recent_fraction > 0"))
    ...
    recent_count = min(
        Int(ceil(actual_batch_size * recent_fraction)),
        length(recent_pool),
        actual_batch_size,
    )

    chosen_recent = sample_without_replacement(recent_pool, recent_count, rng)
    chosen_history = sample_without_replacement(
        history_pool,
        actual_batch_size - length(chosen_recent),
        rng,
    )
    ...
    if length(chosen_recent) + length(chosen_history) < actual_batch_size
        ...
        append!(
            ...
            sample_without_replacement(
                remaining_pool,
                actual_batch_size - length(chosen_recent) - length(chosen_history),
                rng,
            ),
        )
    end
